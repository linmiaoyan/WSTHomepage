...
2. 车牌、网络密码、校章申请进入审批队列，默认状态为「待审核」。管理员在「管理中心」选择对应审批队列，查看详情后通过或驳回。
3. 周期请假由教师提交后自动在智校云联发起申请；这里的「已发起/生效」仅代表系统已成功提交到智校云联，不代表请假审批已通过。
4. 管理员通过其他类型申请后系统会调用对应外部接口并记录执行结果；校章申请会自动生成盖章后 PDF。
5. 教师在「我的申请与办结结果」查看进度；校章申请通过后可下载盖章 PDF 和校章回执。
6. 只有校章 PDF 申请生成回执；学生请假、车牌、网络密码申请不生成办结回执。

> 周期请假由教师提交后自动在智校云联发起申请；这里的“已发起/生效”仅代表系统已成功提交到智校云联，不代表请假审批已通过。如需改回人工审核后再提交，可在 `.env` 设置 `QUEUE_AUTO_APPROVE_LEAVE=0`。
